...
const TinkoffPaymentService_1 = require("../services/TinkoffPaymentService");
const AlfabankPaymentService_1 = require("../services/AlfabankPaymentService");
...
class BillingController {
    ...
    static async getBalance(req, res) {
        try {
            ...
            const wallet = await WalletService_1.WalletService.getOrCreateWallet(req.user.userId);
            return res.json({
    ...
    static async getTransactions(req, res) {
        try {
            ...
            const transactions = await WalletService_1.WalletService.getTransactions(req.user.userId, {
    ...
    static async initPayment(req, res) {
        try {
            ...
            const { amount, paymentSystem, cardType } = req.body;
            // Валидация
            ...
            // Создаём транзакцию
            const transaction = await WalletService_1.WalletService.createTransaction({
            ...
            if (paymentSystem === 'tinkoff') {
                const tinkoff = new TinkoffPaymentService_1.TinkoffPaymentService();
                ...
                // Сохраняем paymentId
                await WalletService_1.WalletService.updateTransaction(transaction.id, {
                    paymentId: payment.paymentId,
                });
            }
            else {
                const alfa = new AlfabankPaymentService_1.AlfabankPaymentService();
        ...
        try {
            ...
            if (system === 'tinkoff') {
                ...
                if (data.Status === 'CONFIRMED') {
                    await WalletService_1.WalletService.completeTransaction(data.OrderId, {
                        amount: data.Amount / 100,
                    });
                }
                else if (data.Status === 'REJECTED' || data.Status === 'CANCELED') {
                    await WalletService_1.WalletService.updateTransaction(data.OrderId, {
                        status: 'failed',
                    });
                }
            }
            else if (system === 'alfabank') {
                const alfabank = new AlfabankPaymentService_1.AlfabankPaymentService();
                ...
                if (data.status === 1) {
                    // Оплачено
                    await WalletService_1.WalletService.completeTransaction(data.mdOrder, {
                        amount: data.amount / 100,
                    });
                }
                else if (data.status === 2) {
                    // Отменено
                    await WalletService_1.WalletService.updateTransaction(data.mdOrder, {
                        status: 'cancelled',
                    });
                }
            }
    ...
    static async checkPaymentStatus(req, res) {
        try {
            ...
            const transaction = await WalletService_1.WalletService.getTransaction(paymentId);
            if (!transaction) {
    ...
    static async generateInvoice(req, res) {
        try {
            ...
            const invoice = await InvoiceService_1.InvoiceService.generateInvoice({
    ...
    static async getInvoices(req, res) {
        try {
            ...
            const invoices = await InvoiceService_1.InvoiceService.getInvoices(req.user.userId, {
                limit: limit ? parseInt(limit) : 50,
                offset: offset ? parseInt(offset) : 0,
                status: status,
            });
    ...
    static async downloadInvoicePDF(req, res) {
        try {
            ...
            const { id } = req.params;
            const invoice = await InvoiceService_1.InvoiceService.getInvoice(id, req.user.userId);
            const pdfBuffer = await InvoiceService_1.InvoiceService.generatePDF(invoice);
     ...
     */
    static async payInvoiceFromWallet(req, res) {
        try {
            if (!req.user) {
                return res.status(401).json({ error: 'Unauthorized' });
            }
            const { id } = req.params;
            const invoice = await InvoiceService_1.InvoiceService.payInvoiceFromWallet(id, req.user.userId);
            return res.json({
                success: true,
                invoice,
            });
     ...
     */
    static async getPricing(_req, res) {
        try {
            const { db } = require('../config/database');
            const plans = await db.manyOrNone('SELECT * FROM pricing_plans WHERE is_active = true ORDER BY created_at DESC');
            return res.json({
                plans: plans || [],
            });
        }
