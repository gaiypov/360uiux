...
class VacancyController {
    ...
    static async createVacancy(req, res) {
        try {
            ...
            const { title, profession, video_url, thumbnail_url, salary_min, salary_max, currency = 'RUB', city, metro, schedule, requires_experience = false, benefits, requirements, tags, } = req.body;
            ...
            // Создаём вакансию
            const vacancy = await DatabaseService_1.db.one(`INSERT INTO vacancies (
          ...
          'draft', 0, 0, false
            ...
            await CacheService_1.cacheService.invalidateVacancies();
    ...
    static async listVacancies(req, res) {
        try {
            const { page = '1', limit = '20', city, profession, salary_min, salary_max, schedule, requires_experience, sort = 'created_at', // created_at | salary_min | salary_max
            ...
            const cached = await CacheService_1.cacheService.getVacancyList(filters);
            if (cached) {
      ...
      `;
            ...
            const results = await DatabaseService_1.db.manyOrNone(query, params);
            const vacancies = results.map((row) => {
            ...
            await CacheService_1.cacheService.cacheVacancyList(filters, response.data);
    ...
    static async getVacancy(req, res) {
        try {
            const { id } = req.params;
            // Попытка получить из кэша
            const cached = await CacheService_1.cacheService.getVacancy(id);
            if (cached) {
                console.log(`🚀 Vacancy ${id} served from cache`);
                // Инкремент просмотров асинхронно (не блокируя ответ)
                if (cached.status === 'active') {
                    DatabaseService_1.db.none('UPDATE vacancies SET views = views + 1 WHERE id = $1', [id]).catch((err) => console.error('Failed to increment views:', err));
                }
            ...
            const vacancy = await DatabaseService_1.db.oneOrNone(`SELECT
          v.*,
          u.company_name,
          u.inn,
          u.verified as employer_verified,
        ...
        WHERE v.id = $1`, [id]);
            ...
            if (vacancy.status === 'active') {
                await DatabaseService_1.db.none('UPDATE vacancies SET views = views + 1 WHERE id = $1', [id]);
                vacancy.views = (vacancy.views || 0) + 1;
            }
            // Сохранить в кэш на 1 час
            await CacheService_1.cacheService.cacheVacancy(id, vacancy);
            return res.status(200).json({
                success: true,
                vacancy,
            });
    ...
    static async updateVacancy(req, res) {
        try {
            ...
            const existing = await DatabaseService_1.db.oneOrNone('SELECT * FROM vacancies WHERE id = $1', [id]);
            if (!existing) {
                return res.status(404).json({
                    error: 'Not Found',
                    message: 'Vacancy not found',
            ...
            const { title, profession, video_url, thumbnail_url, salary_min, salary_max, currency, city, metro, schedule, requires_experience, benefits, requirements, tags, status, } = req.body;
            // Валидация статуса
            ...
            // Обновление
            const updated = await DatabaseService_1.db.one(`UPDATE vacancies SET
        ...
        RETURNING *`, [
            ...
            await CacheService_1.cacheService.invalidateVacancy(id);
            return res.status(200).json({
                success: true,
                vacancy: updated,
            });
    ...
    static async deleteVacancy(req, res) {
        try {
            ...
            // Проверка владения
            const vacancy = await DatabaseService_1.db.oneOrNone('SELECT * FROM vacancies WHERE id = $1', [id]);
            if (!vacancy) {
                return res.status(404).json({
                    error: 'Not Found',
                    message: 'Vacancy not found',
            ...
            await DatabaseService_1.db.none(`UPDATE vacancies SET
          status = 'archived',
          updated_at = NOW()
        WHERE id = $1`, [id]);
     ...
     */
    static async publishVacancy(req, res) {
        try {
            const user = req.user;
            const { id } = req.params;
            if (!user || user.role !== 'employer') {
            ...
            const vacancy = await DatabaseService_1.db.oneOrNone('SELECT * FROM vacancies WHERE id = $1 AND employer_id = $2', [id, user.userId]);
            if (!vacancy) {
                return res.status(404).json({
                    error: 'Not Found',
     ...
     * Стоимость: 500₽
     */
    static async extendVacancy(req, res) {
        try {
            const user = req.user;
            const { id } = req.params;
            if (!user || user.role !== 'employer') {
            ...
            const wallet = await DatabaseService_1.db.oneOrNone('SELECT * FROM company_wallets WHERE employer_id = $1', [user.userId]);
            ...
            await DatabaseService_1.db.transaction(async (client) => {
                // Списать средства
                await client.query('UPDATE company_wallets SET balance = balance - $1, updated_at = NOW() WHERE employer_id = $2', [EXTEND_COST, user.userId]);
                // Создать транзакцию
            ...
            gen_random_uuid(), $1, 'payment', $2, 'RUB', 'completed', 'Vacancy extend', NOW()
                ...
                // Продлить вакансию на 7 дней
                await client.query(`UPDATE vacancies SET
            published_at = CASE
          ...
          WHERE id = $1`, [id]);
            ...
            const updated = await DatabaseService_1.db.one('SELECT * FROM vacancies WHERE id = $1', [id]);
