...
class ApplicationController {
    ...
    static async createApplication(req, res) {
        try {
            ...
            const vacancy = await DatabaseService_1.db.oneOrNone('SELECT * FROM vacancies WHERE id = $1 AND status = $2', [vacancyId, 'active']);
            if (!vacancy) {
                return res.status(404).json({ error: 'Vacancy not found or not active' });
            }
            // Проверить что уже не откликнулись
            const existingApplication = await DatabaseService_1.db.oneOrNone('SELECT * FROM applications WHERE vacancy_id = $1 AND jobseeker_id = $2', [vacancyId, jobseekerId]);
            ...
            if (attachResumeVideo) {
                if (resumeId) {
                    ...
                    const resume = await DatabaseService_1.db.oneOrNone('SELECT * FROM resumes WHERE id = $1 AND jobseeker_id = $2', [resumeId, jobseekerId]);
                    ...
                    const video = await DatabaseService_1.db.oneOrNone('SELECT * FROM videos WHERE user_id = $1 AND type = $2', [jobseekerId, 'resume']);
                    if (video) {
            ...
            const application = await DatabaseService_1.db.one(`INSERT INTO applications (
        ...
        RETURNING *`, [vacancyId, jobseekerId, resumeId || null, message || null, resumeVideoId, chatRoomId]);
            // Создать системное сообщение в чате
            await chatService.createSystemMessage(application.id, 'Соискатель откликнулся на вашу вакансию');
            ...
            await DatabaseService_1.db.none('UPDATE vacancies SET applications_count = applications_count + 1 WHERE id = $1', [vacancyId]);
    ...
    static async getMyApplications(req, res) {
        try {
            ...
            const applications = await DatabaseService_1.db.manyOrNone(`SELECT
          a.*,
     ...
     */
    static async getVacancyApplications(req, res) {
        try {
            ...
            const vacancy = await DatabaseService_1.db.oneOrNone('SELECT * FROM vacancies WHERE id = $1 AND employer_id = $2', [vacancyId, employerId]);
            if (!vacancy) {
    ...
    static async updateStatus(req, res) {
        try {
            ...
            const application = await DatabaseService_1.db.oneOrNone(`SELECT a.*, v.employer_id
         FROM applications a
         JOIN vacancies v ON v.id = a.vacancy_id
         WHERE a.id = $1`, [id]);
            if (!application) {
            ...
            // Обновить статус
            const updated = await DatabaseService_1.db.one(`UPDATE applications
         SET employer_status = $1,
             ...
             updated_at = NOW()
         WHERE id = $3
         RETURNING *`, [employerStatus, notes || null, id]);
    ...
    static async getApplication(req, res) {
        try {
            ...
            const application = await DatabaseService_1.db.oneOrNone(`SELECT
          a.*,
          v.title as vacancy_title,
          v.employer_id,
          u.name as jobseeker_name,
    ...
    static async deleteApplication(req, res) {
        try {
            ...
            // Проверить владельца
            const application = await DatabaseService_1.db.oneOrNone('SELECT * FROM applications WHERE id = $1 AND jobseeker_id = $2', [id, userId]);
            if (!application) {
                return res.status(404).json({ error: 'Application not found' });
            }
            // Удалить (cascade удалит chat messages)
            await DatabaseService_1.db.none('DELETE FROM applications WHERE id = $1', [id]);
            // Уменьшить счетчик откликов
            await DatabaseService_1.db.none('UPDATE vacancies SET applications_count = applications_count - 1 WHERE id = $1', [application.vacancy_id]);
