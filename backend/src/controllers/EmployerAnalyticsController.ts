...
export class EmployerAnalyticsController {
  ...
  static async getDashboard(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Get total stats
      const totalStats = await db.oneOrNone(`
        SELECT
      ...
      const viewsData = await db.manyOrNone(`
        SELECT
      ...
      const applicationsData = await db.manyOrNone(`
        SELECT
      ...
      const previousStats = await db.oneOrNone(`
        SELECT
      ...
      res.json({
        ...
        analytics: {
          ...
          changes: {
            views: parseFloat(viewsChange as string),
            applications: parseFloat(applicationsChange as string),
          },
