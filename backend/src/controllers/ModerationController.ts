...
export class ModerationController {
   ...
   */
  static async getPendingVideos(req: Request, res: Response) {
    try {
      ...
      const totalResult = await db.one<{ count: string }>(
        `SELECT COUNT(*) as count FROM videos WHERE ${whereConditions}`
      );
      const total = parseInt(totalResult.count);

      // Получение видео
      const videos = await db.manyOrNone<Video>(
        `SELECT v.*, u.phone, u.name, u.company_name
         FROM videos v
      ...
      const response: GetPendingVideosResponse = {
        videos: videos || [],
        total,
        page: Number(page),
        limit: Number(limit),
   ...
   * Доступ: только модераторы
   */
  static async moderateVideo(req: Request, res: Response) {
    try {
      const { video_id, action, reason, comment }: ModerateVideoRequest = req.body;
      ...
      const updatedVideo = await db.one<Video>(
        `UPDATE videos
         ...
         WHERE id = $5
         RETURNING *`,
        [newStatus, moderationStatus, moderatorId, reason || null, video_id]
      );

      // Создание лога модерации
      await db.none(
        `INSERT INTO moderation_logs (video_id, action, performed_by, comment, details)
         VALUES ($1, $2, $3, $4, $5)`,
        [
          video_id,
  ...
  static async submitComplaint(req: Request, res: Response) {
    try {
      const { video_id, reason, description }: SubmitComplaintRequest = req.body;
      ...
      const existingComplaint = await db.oneOrNone<VideoComplaint>(
        'SELECT * FROM video_complaints WHERE video_id = $1 AND user_id = $2',
        [video_id, userId]
      );
      ...
      // Создание жалобы
      const complaint = await db.one<VideoComplaint>(
        `INSERT INTO video_complaints (video_id, user_id, reason, description)
         VALUES ($1, $2, $3, $4)
         RETURNING *`,
        [video_id, userId, reason, description || null]
      );

      // Создание лога
      await db.none(
        `INSERT INTO moderation_logs (video_id, action, performed_by, details)
         VALUES ($1, $2, $3, $4)`,
        [
          video_id,
          'complaint_received',
   ...
   * Доступ: только модераторы
   */
  static async reviewComplaint(req: Request, res: Response) {
    try {
      const { complaintId } = req.params;
      const { status, comment }: ReviewComplaintRequest = req.body;
      const moderatorId = req.user!.userId;
      ...
      const complaint = await db.one<VideoComplaint>(
        `UPDATE video_complaints
         ...
         WHERE id = $4
         RETURNING *`,
        [status, moderatorId, comment || null, complaintId]
      );
   ...
   */
  static async performAICheck(req: Request, res: Response) {
    try {
      ...
      await db.none(
        `INSERT INTO moderation_logs (video_id, action, details)
         VALUES ($1, $2, $3)`,
        [videoId, 'ai_check_started', JSON.stringify({ video_url: video.player_url })]
      );

      // TODO: Интеграция с AWS Rekognition или другим AI сервисом
      // Пока используем mock результат
      const aiResult: AICheckResult = await this.mockAICheck(video);
      ...
      if (req.params.videoId) {
        await db.none(
          `INSERT INTO moderation_logs (video_id, action, details)
           VALUES ($1, $2, $3)`,
          [req.params.videoId, 'ai_check_failed', JSON.stringify({ error: error.message })]
        ).catch(() => {});
      }
   ...
   * Доступ: только модераторы
   */
  static async getModerationLogs(req: Request, res: Response) {
    try {
      const { videoId } = req.params;

      const logs = await db.manyOrNone<ModerationLog>(
        `SELECT ml.*,
   ...
   */
  private static async mockAICheck(video: Video): Promise<AICheckResult> {
