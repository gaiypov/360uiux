...
export class VacancyController {
   ...
   */
  static async searchVacancies(req: Request, res: Response) {
    try {
      const {
        query = '',
        page = '1',
        ...
        schedule,
        requires_experience,
        sort = 'relevance', // relevance | created_at | salary_min | salary_max
      } = req.query;
      ...
      if (cached) {
        ...
        return res.status(200).json(JSON.parse(cached) as any);
      }
